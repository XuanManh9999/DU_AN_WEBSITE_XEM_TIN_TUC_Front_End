...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-50 transition-all duration-300">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between py-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={searchRef}>
              <div className={`flex items-center transition-all duration-300 ${isSearchExpanded ? 'bg-white rounded-lg shadow-md border border-gray-200' : ''}`}>
                {isSearchExpanded ? (
                      ...
                      <>
                          ...
                          {searchResults.map((article, index) => (
                            <li key={article.id} className="mb-1">
                              <Link
                                ...
                                className={`flex items-center gap-3 p-2 rounded-lg transition-all duration-300 hover:bg-gray-50 no-underline ${selectedIndex === index ? 'bg-blue-50 border-l-4 border-blue-500' : ''
                                  }`}
                              >
                                <img
                        ...
                        {searchResults.length === 5 && (
                          <div className="border-t border-gray-100 mt-2 pt-2">
                            <Link
                              to={`/tim-kiem?q=${encodeURIComponent(searchTerm)}`}
            ...
            {isLogin ? (
              ...
              <div className="relative hidden md:block" ref={userMenuRef}>
                ...
                {/* User Dropdown */}
                <div
                  className={`absolute top-full right-0 mt-2 w-56 bg-white rounded-lg shadow-2xl py-2 transition-all duration-300 z-[9999] ${isUserMenuOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-3'
                    }`}
            ...
            <div className="bg-red-600 text-white px-3 py-2 rounded-lg shadow-md hover:bg-red-700 transition-all duration-300 flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold">1900 6489</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="border-t border-gray-200">
          <ul className="flex justify-center items-center gap-4 list-none m-0 py-3">
            {/* Home Menu */}
            <li>
              <Link to="/" className="text-gray-700 hover:text-blue-600 no-underline font-medium text-sm transition-all duration-300 flex items-center gap-1">
            ...
            {dataCategories?.slice(0, 6).map((item, index) => (
              ...
              >
                ...
                >
                  ...
                  {item?.children && (
                    <span className={`ml-1 text-xs transition-transform duration-300 ${activeDropdown === index ? 'rotate-180' : ''}`}>
                      ▼
                    </span>
                  )}
                </Link>
                {item?.children && (
                  <ul className={`absolute top-full left-0 bg-white rounded-lg shadow-2xl list-none m-0 py-2 min-w-[180px] z-50 transition-all duration-300 ${activeDropdown === index
      ...
      {/* Mobile Navigation */}
      <nav className={`md:hidden bg-white border-b border-gray-200 transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
        <div className="p-4">
          {/* Mobile Search */}
          <div className="mb-4 relative">
            <form onSubmit={handleSearchSubmit} className="relative flex items-center bg-gray-100 rounded-lg overflow-hidden">
              <input
                type="text"
                      ...
                      <ul className="list-none m-0 p-0">
                        {searchResults.map((article, index) => (
                          <li key={article.id} className="mb-2">
                            <a
                              href={`/${article.slugCategory}/${article.slug || article.id}`}
            ...
            <div className="mb-4 pb-4 border-b border-gray-200">
              ...
              {userMenuItems.map((item, index) => (
                <Link
                  ...
                  className={`flex items-center gap-3 py-3 text-sm transition-all duration-300 ${item.isLogout ? 'text-red-600' : 'text-gray-700 hover:text-blue-600'
                    }`}
                >
                  {item.icon}
                  {item.title}
