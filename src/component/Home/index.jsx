...
import { getAllArticles, getArticleByCategoryIdLimit4 } from "../../services/articles";
...
export default function Home() {
  ...
  const fetchLatestNews = async (page = 1) => {
    try {
      ...
      const response = await getAllArticles(SIDEBAR_ITEMS_PER_PAGE, offset, "createAt", "desc");
  ...
  const fetchPostOutstandings = async (page = 1) => {
    try {
      ...
      const response = await getAllArticles(ITEMS_PER_PAGE, offset, "view", "desc");
      if (response && response.status === 200) {
  ...
  const sidebarTotalPages = Math.ceil(allSidebarNews.length / SIDEBAR_ITEMS_PER_PAGE);
  const newsListTotalPages = Math.ceil(allNewsList.length / ITEMS_PER_PAGE);
  const popularNewsTotalPages = Math.ceil(allPopularNews.length / ITEMS_PER_PAGE);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {tabArticles?.map((news) => (
                      <div key={news.id} className="flex gap-4 group cursor-pointer">
                        <Link to={`/${news?.slugCategory}/${news.slug}`} className="flex gap-4 w-full">
                          <img
                            src={news?.thumbnail || ""}
                            alt=""
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="bg-gradient-to-r from-red-500 to-red-600 rounded-lg p-6 text-white">
                ...
                <p className="text-sm opacity-90">
                  Trang web cập nhật tin tức mới nhất, đa dạng lĩnh vực: Thời sự, Thể thao, Công nghệ, Giải trí, Kinh doanh, Đời sống...
                </p>
                <p className="text-sm opacity-90">
                  Nơi bạn có thể tìm kiếm, khám phá và chia sẻ những thông tin hữu ích mỗi ngày.
                </p>
              </div>
              <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-6 text-white">
                <h3 className="font-bold text-lg mb-2">Khám phá trải nghiệm mới</h3>
                <p className="text-sm opacity-90">
                  Đăng ký tài khoản để nhận thông báo, lưu bài viết yêu thích và tham gia bình luận cùng cộng đồng.
                </p>
                <div className="mt-4">
          ...
          <div className="lg:col-span-1">
              ...
              {!loading && !error && (
                <div className="border-t border-gray-200">
                  <Pagination
                    ...
                    totalPages={Math.ceil(totalLatestNews / SIDEBAR_ITEMS_PER_PAGE)}
            ...
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-green-400 to-green-600 rounded-lg p-4 shadow-md">
                <h4 className="font-bold text-white mb-2">THÔNG BÁO QUAN TRỌNG</h4>
                <p className="text-sm text-white opacity-90">Lịch bảo trì hệ thống cấp nước toàn tỉnh vào ngày 15/07/2024. Quý khách vui lòng dự trữ nước và theo dõi thông báo mới nhất trên website.</p>
              </div>
              <div className="bg-gradient-to-r from-pink-500 to-pink-700 rounded-lg p-4 shadow-md">
                <h4 className="font-bold text-white mb-2">CHƯƠNG TRÌNH KHUYẾN MÃI</h4>
                <p className="text-sm text-white opacity-90">Giảm 10% cho khách hàng đăng ký dịch vụ nước trực tuyến trong tháng 7. Đăng ký ngay để nhận ưu đãi!</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white py-8 px-4 rounded-lg shadow-md mt-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            ...
            <div>
              ...
              <div className="text-orange-500 font-semibold text-sm mb-1">Hotline 1900.636.616</div>
              <div className="font-bold mb-1">HỖ TRỢ TRỰC TUYẾN</div>
              <div className="text-gray-600 text-sm mb-2">
                Tư vấn trực tuyến, hỗ trợ đăng ký lắp đặt, sửa chữa nước. Hỗ trợ sử dụng và bảo hành đồng hồ. Tư vấn điều chỉnh giá bán, giải đáp mọi thắc mắc.
              </div>
            ...
            <div>
              ...
              <div className="text-gray-600 text-sm">
                Từ 2018, BAWACO đã triển khai sử dụng hóa đơn điện tử thay thế hóa đơn giấy. Khách hàng tra cứu trực tuyến hóa đơn tiền nước tại địa bàn tỉnh Bà Rịa-Vũng Tàu.
              </div>
            </div>
            ...
            <div>
              ...
              <div className="text-gray-600 text-sm">
                Khách hàng BAWACO có thể thanh toán hóa đơn nước trực tuyến, qua các ngân hàng liên kết hoặc ví điện tử VNPAY/Pay Bills/Website của nhà cung cấp.
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden mt-8">
          <div className="p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-lg text-blue-600">Bài viết phổ biến</h3>
              <div className="flex bg-gray-100 rounded-lg p-1">
                <button
                  ...
                  className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${activePopularTab === 'views'
                ...
                <button
                  ...
                  className={`px-3 py-1 text-sm font-medium rounded-md transition-colors ${activePopularTab === 'likes'
              ...
              {loading ? (
                ...
                postOutstandings.map((news) => (
                  <div key={news.id} className="flex gap-4 group cursor-pointer pb-4 border-b border-gray-100 last:border-b-0">
                    <Link to={`/${news?.slugCategory}/${news.slug || news.id}`} className="flex gap-4 w-full">
                      <img
                        src={news?.thumbnail || ""}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                          <span>{formatDate(news?.createAt)}</span>
                          <span>•</span>
                          <span className={`flex items-center gap-1 ${activePopularTab === 'views' ? 'text-blue-600 font-medium' : ''}`}>
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                              <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                              <path fillRule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clipRule="evenodd" />
                            </svg>
                          ...
                          <span>•</span>
                          <span className={`flex items-center gap-1 ${activePopularTab === 'likes' ? 'text-red-600 font-medium' : ''}`}>
                            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                              <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
                            </svg>
                            {news?.quantityLike || 0}
